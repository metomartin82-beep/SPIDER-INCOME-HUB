...
// Shared by both the typed-code flow (/verify) and the clickable-link flow
// (/verify-link) so the reward/badge/admin logic can't drift between the two.
const completeVerification = async (user) => {
  await db.execute({
    sql: 'UPDATE users SET is_verified = 1, otp_code = NULL, otp_expires = NULL, verify_token = NULL, verify_token_expires = NULL WHERE id = ?',
    args: [user.id]
  });
  await rewardReferrer(user.referred_by, user.id);
  await awardBadge(user.id, 'badge_welcome');
  await awardBadge(user.id, 'badge_verified');
...
router.post('/register', async (req, res) => {
  try {
    ...
    const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); // 24h — long enough for someone to get to their email
    const verifyToken = crypto.randomBytes(32).toString('hex');
    const myReferralCode = generateReferralCode(username);

    await db.execute({
      sql: `INSERT INTO users (id, username, email, password, otp_code, otp_expires, verify_token, verify_token_expires, referral_code, referred_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, username, email.toLowerCase(), hashedPassword, otp, expiresAt, verifyToken, expiresAt, myReferralCode, referredBy]
    });
    ...
    res.status(201).json({ message: 'Registered! Check your email for a verification link (or code).', userId: id });
  } catch (err) {
    console.error(err);
...
// GET /api/auth/verify-link — clicked from the email button. Verifies the
// account, then redirects back into the app rather than returning JSON,
// since a browser navigates here directly (not an API call from the SPA).
