  ...
  try {
    ...
    await db.execute({
      sql: `INSERT INTO bot_listings (id, seller_id, name, description, category, cover_image, download_url, demo_url, price_cops, rental_price_cops_per_day)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, req.user.id, name.trim(), description || '', category || 'whatsapp', imgCheck.value, downloadUrl, demoUrl || '', parseInt(priceCOPS, 10), parseInt(rentalPricePerDay, 10) || 0]
    });
...
router.put('/listings/:id', protect, async (req, res) => {
  try {
    ...
    if (coverImage !== undefined) {
      const imgCheck = validateImageUrl(coverImage, { fieldName: 'Cover image' });
      if (!imgCheck.ok) return res.status(400).json({ message: imgCheck.error });
      coverImageValue = imgCheck.value;
    }
    await db.execute({
      sql: `UPDATE bot_listings SET name = COALESCE(?, name), description = COALESCE(?, description),
            ...
            WHERE id = ?`,
      args: [name, description, coverImageValue, downloadUrl, demoUrl, priceCOPS ? parseInt(priceCOPS, 10) : null,
