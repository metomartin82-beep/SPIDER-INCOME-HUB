...
// Uses whatever key is pasted into TMDB_API_KEY — no built-in fallback, so
// posters just get skipped gracefully if the env var isn't set, rather than
// silently sharing a public demo key across projects.
const TMDB_API_KEY = process.env.TMDB_API_KEY || '';
const posterCache = {};
async function getPoster(title, year) {
  if (!TMDB_API_KEY) return ''; // no key configured — skip poster lookup entirely
  const key = `${title}-${year}`;
  if (posterCache[key] !== undefined) return posterCache[key];
  try {
    const q   = encodeURIComponent(cleanTitle(title));
    const url = `https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${q}&year=${year}&language=en-US&page=1`;
