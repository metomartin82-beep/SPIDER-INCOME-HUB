...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const mediaCheck = validateImageUrl(mediaUrl, { fieldName: 'Media' });
    if (!mediaCheck.ok) return res.status(400).json({ message: mediaCheck.error });
    ...
    await db.execute({
      ...
      args: [id, req.user.id, content, mediaCheck.value, groupId || null, pollData]
    });
