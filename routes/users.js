...
router.put('/me', protect, async (req, res) => {
  try {
    ...
    if (avatar !== undefined) {
      const check = validateImageUrl(avatar, { fieldName: 'Avatar' });
...
router.put('/me/cover', protect, async (req, res) => {
  try {
    ...
    const check = validateImageUrl(coverPhoto, { fieldName: 'Cover photo' });
    if (!check.ok) return res.status(400).json({ message: check.error });
    await db.execute({ sql: 'UPDATE users SET cover_photo = ? WHERE id = ?', args: [check.value, req.user.id] });
    res.json({ message: 'Cover photo updated', coverPhoto: check.value });
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
